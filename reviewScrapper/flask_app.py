    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if reviews.count() > 0: # if there is a collection with searched keyword and it has records in it
                ...
                bigboxes = flipkart_html.findAll("div", {"class": "lvJbLV col-12-12"}) # seacrhing for appropriate tag to redirect to the product link
                del bigboxes[0:3] # the first 3 members of the list do not contain relevant information, hence deleting them.
                box = bigboxes[0] #  taking the first iteration (for demo)
                productLink = "https://www.flipkart.com" + box.div.div.div.a['href'] # extracting the actual product link
                prodRes = requests.get(productLink) # getting the product page from server
                prod_html = bs(prodRes.text, "html.parser") # parsing the product page as HTML
                commentboxes = prod_html.find_all('div', {'class': "x_CUu6"}) # finding the HTML section containing the customer comments
                ...
                for commentbox in commentboxes:
                    try:
                        name = commentbox.div.div.find_all('p', {'class': 'zJ1ZGa ZDi3w2'})[0].text
...
if __name__ == "__main__":
    app.run(debug=True , port=5001, use_reloader=False) # running the app on the local machine on port 8000